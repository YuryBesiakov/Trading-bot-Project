...
class TradingBot:
    ...
    def __init__(self, config: Dict[str, any]):
        ...
        raw_strategy_cfg: Dict[str, any] = config.get("strategy", {})
        enable = raw_strategy_cfg.get("enable", {})
        rsi_cfg = raw_strategy_cfg.get("rsi", {})
        # Patterns
        patterns = {
            "bullish_engulfing": enable.get("engulfing", False),
            "bearish_engulfing": enable.get("engulfing", False),
            "hammer": enable.get("hammer", False),
            "gap_up": enable.get("gap", False),
            "gap_down": enable.get("gap", False),
        }
        # Indicators
        indicators = {
            "rsi": enable.get("rsi", False),
            "sma_50": enable.get("moving_averages", False),
            "sma_200": enable.get("moving_averages", False),
            "ema_50": enable.get("moving_averages", False),
            "ema_200": enable.get("moving_averages", False),
            "golden_cross": enable.get("moving_averages", False),
            "death_cross": enable.get("moving_averages", False),
        }
        # Thresholds
        thresholds = {
    ...
    def start(self):
        ...
        self.run_analysis()        # 👈 force one run immediately
        scheduler.start()
